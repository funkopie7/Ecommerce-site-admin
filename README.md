...
Public catalog is available at `/api/catalog/products`. Admin mutations require the `x-admin-key` header. Customer sessions are HTTP-only cookies. Checkout is intentionally a simulated payment; it reduces stock and writes immutable item/address snapshots in one database transaction.
