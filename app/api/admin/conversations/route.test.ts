...
it("lists every thread with its unread count", async () => {
  db.conversation.findMany.mockResolvedValueOnce([{ id: "c1", status: "OPEN", customer: { id: "cust1", name: "Ansh", email: "a@b.com" }, messages: [{ body: "hi", sender: "CUSTOMER", createdAt: new Date() }] }]);
